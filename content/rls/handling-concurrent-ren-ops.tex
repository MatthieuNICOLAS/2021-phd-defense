  ...
  \onslide<4->{
    \begin{block}{Intuition}
      \begin{enumerate}
        ...
        \item<6-> $\trm{id}$ a été inséré après le renommage : doit retourner une valeur qui préserve l'ordre
        \item<7->
          \only<7,8>{$\trm{id}$ a été inséré après ou en concurrence : doit restaurer son ancienne valeur ou générer nouvelle valeur}
          \only<9>{$\trm{id}$ a été inséré après ou en concurrence : doit restaurer son ancienne valeur ou générer nouvelle valeur}
      \end{enumerate}
    \end{block}
  }
\end{frame}
