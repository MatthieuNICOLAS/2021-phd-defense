...
\begin{frame}{Algorithme d'intégration d'une opération \ren}
  \begin{block}{Intuition}
    \begin{enumerate}
      ...
      \begin{enumerate}
        \item<5-> \only<8,9>{\color{gray}}Calculer chemin entre époque courante et époque cible, et notamment leur Plus Proche Ancêtre Commun (PPAC)
        \item<6-> Annuler l'effet des opérations \ren de l'époque courante au PPAC
        \item<7-> \only<9>{\color{gray}}Appliquer l'effet des opérations \ren du PPAC à l'époque cible
      \end{enumerate}
    \end{enumerate}
  \end{block}
\end{frame}
  ...
  \begin{figure}
    \resizebox{\textwidth}{!}{
      \begin{tikzpicture}
        ...
        \onslide<2->{
          \path
            (a-ren-A2) node[point, label=above:{$\trm{ren}(\epoch{0}, A,2)$}] {};
          \draw<2->[->, thick] (a-initial) --  (a-ren-A2) -- (a-final);
        }
        \draw<3-5>[->, thick] (b-initial) -- (b-ren-B3) -- (b-final);
        \onslide<3-> {
          \path
            (b-ren-B3) node[point, label=below:{$\trm{ren}(\epoch{0},B,3)$}] {};
        }
        ...
        \onslide<5-> {
          \path
            (c-ren-c6) node[point, label=below:{$\trm{ren}(\epoch{A2},C,6)$}] {};
          \draw[->, thick] (c-initial) -- (c-recv-ren-A2) -- (c-ren-c6) -- (c-final);
        }
        \onslide<6-> {
          \path
            (b-ren-b7) node[point, label=below:{$\trm{ren}(\epoch{B3},B,7)$}] {};
        ...
        \begin{figure}
          \resizebox{\columnwidth}{!}{
            \begin{tikzpicture}[scale=0.8,every node/.style={scale=0.3}]
              ...
              \onslide<9>{
                \path
                  (eB7) node[op, red] (eB7-node-2) {$\epoch{B7}$};
                \draw[->] (eB3-node) edge (eB7-node-2);
                \draw[->, dashed, red] (eB7-node-2.135) -- (eB3-node.225) (eB3-node.180) -- (eC6-node.45) -- (eA2-node.315) (eA2-node.0) -- (e0.270);
              }
            \end{tikzpicture}
          }
        \end{figure}
    ...
    \begin{column}{0.7 \textwidth}
      ...
      \begin{block}<7->{Comment choisir ?}
        \begin{itemize}
          ...
          \item<9> Utilise ordre lexicographique sur chemins des époques dans l'arbre
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
