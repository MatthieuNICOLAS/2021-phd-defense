        ...
        \resizebox{\textwidth}{!}{
          \begin{tikzpicture}
            ...
            \path
                ...
                +(0:14) node (a-final) {};
            \path<5>
                (a-ren-a1) node[label={[xshift=0pt]-10:{$\trm{ren}(\epoch{0}, A,2)$}}] {};
...
\begin{frame}[fragile]{Gestion des opérations \ins et \rmv concurrentes}
  \vspace{-1.5em}
  \begin{figure}
    \resizebox{\textwidth}{!}{
      \begin{tikzpicture}
...
\begin{frame}[fragile]{Exemple de \texttt{renameId}}
    ...
    \begin{block}{Exemple avec {$\coloridthree\id{i}{B1}{0}\id{m}{B2}{0}$}}
      \begin{itemize}
        \item<2-> Trouver son prédecesseur à l'époque d'origine $\epoch{0}$ : $\coloridone\id{i}{B1}{0}\id{f}{A1}{0}$
        \item<3-> Trouver son équivalent à l'époque cible $\epoch{A2}$ : $\coloridtwo\id{i}{A2}{1}$
        \item<4-> Concaténer ce dernier à {$\coloridthree\id{i}{B1}{0}\id{m}{B2}{0}$} pour obtenir son équivalent à $\epoch{A2}$ : $\coloridfour\id{i}{A2}{1}\id{i}{B1}{0}\id{m}{B2}{0}$
      \end{itemize}
    \end{block}
  \end{frame}
